...
obs_f = vphi1_b + 0.2; 
obs=fimplicit(obs_f, 'w');          % do not show the curve
obs_f=fill(obs.XData, obs.YData, 'r');               % obstacle
obs_f.FaceAlpha=0.5;  obs_f.EdgeAlpha=0;
...
obs_f = vphi5_b+0.3; 
obs=fimplicit(obs_f, [-2 0 1.5 2.5], 'w');          % do not show the curve
obs_f=fill(obs.XData, obs.YData, 'r');               % obstacle
obs_f.FaceAlpha=0.5;  obs_f.EdgeAlpha=0;
obs_f = vphi5_b+0.3; 
obs=fimplicit(obs_f, [0 2 1.5 2.5], 'w');          % do not show the curve
obs_f=fill(obs.XData, obs.YData, 'r');               % obstacle
obs_f.FaceAlpha=0.5;  obs_f.EdgeAlpha=0;
